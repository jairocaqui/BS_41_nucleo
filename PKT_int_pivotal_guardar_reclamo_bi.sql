CREATE OR REPLACE PACKAGE WEBRECLAMOS.PKG_INT_PIVOTAL
IS
...
/
CREATE OR REPLACE PACKAGE BODY WEBRECLAMOS.PKG_INT_PIVOTAL
IS
               ...
               /*  AS (SELECT DISTINCT
                      ...
                      WHERE (TO_CHAR (A.FRCPCN, 'MMYYYY') = as_MES || as_ANIO
                            ...
                            /*DECODE (
                               ...
                               1,    LPAD (A.CCRGSTRO, 3, '0')
                                  || '-'
                                  || A.CORGSTRO					//no exiten 
                                  || '-'
                                  || A.ARGSTRO					//no exiten 
                                  || '-'
                                  || LPAD (A.CRGSTRO, 5, '0'),		//no exiten 
                               '')*/
							    DECODE (
                               A.CPRCSO,
                               1,    LPAD (A.CCRGSTRO, 3, '0')
                            ...
                            END
                               TRCLMO                                     --06
                                     ,
                            (SELECT X.NVRCLMO
                               FROM NC_PE_BI.ODS_WRE_SASMT07 X
                              WHERE X.CVRCLMO = A.CVRCLMO)
                               VRCLMO                                     --07
                                     ,
                            /*(SELECT SUBSTR (X.NPRCSO, 1, 20)				--no existe campo
                               FROM NC_PE_BI.ODS_WRE_SASMT04 X
                              WHERE X.CPRCSO = A.CPRCSO)*/
								null as
                               TVRCLMO                                    --08
                                      --RECLAMOS_CONTRATANTE
                            ...
                            C.NSNSTRO NSNSTRO                             --13
                                             ,
                            (SELECT SUBSTR (X.NREMPRSA, 1, 20)
                               FROM NC_PE_BI.ODS_WRE_SASMT22 X
                              WHERE X.CREMPRSA = B.CREMPRSA)
                               REMPRSA                                    --14
                                      ,
                            (SELECT SUBSTR (X.NTUSRO, 1, 100)
                               FROM NC_PE_BI.ODS_WRE_SASMT24 X
                              WHERE X.CTUSRO = B.CTUSRO
                                    AND X.cremprsa = B.cremprsa)
                               TUSRO                                      --15
                                    --RECLAMOS_RECLAMANTE
                            ...
                            (SELECT SUBSTR (X.NOFCNA, 1, 100)
                              ...
                              WHERE X.COFCNA = A.CORGNRDRA)
                               ORGNRDRA            -- OFICINA_GENERADORA, --27
                                       ,
                            (SELECT SUBSTR (X.NARA, 1, 50)
                               FROM NC_PE_BI.ODS_WRE_SASMT09 X
                              WHERE     X.CCMPNA = A.CCRGNRDRA
                                    AND X.CPRCSO = A.CPRCSO
                                    AND X.CARA = A.CARA
                                    AND X.COFCNA = A.CORGNRDRA)
                            ...
                            CASE A.CPRDCTO
                               ...
                               ELSE
                                  (SELECT SUBSTR (NPRDCTO, 1, 100)
                                     FROM NC_PE_BI.ODS_WRE_SASMT16 P
                                    WHERE A.CPRDCTO = P.CPRDCTO
                                          AND ROWNUM = 1)
                            END
                               PRDCTO                                     --29
                            ...
                            CASE A.CSRVCO
                               ...
                               ELSE
                                  (SELECT SUBSTR (X.NSRVCO, 1, 100)
                                     FROM NC_PE_BI.ODS_WRE_SASMT15 X
                                    WHERE A.CSRVCO = X.CSRVCO AND ROWNUM = 1)
                            END
                               SRVCO                                      --30
                                    ,
                            (SELECT X.NMTVO
                               FROM NC_PE_BI.ODS_WRE_SASMT14 X
                              WHERE     X.CMTVO = A.CMTVO
                                    AND X.CCMPNA = A.CCRGNRDRA
                                    AND ROWNUM = 1)
                               MTVO                                       --31
                            ...
                            --SUBSTR (
                              -- REPLACE (
                               ...
                               ROCRRNCA                  --33  DETALLE_RECLAMO
                                       ,
                            (SELECT DISTINCT UPPER (SUBSTR (X.VDTVRA, 1, 50))
                               FROM NC_PE_BI.ODS_WRE_SASMT30 X
                              WHERE     X.CTVRA = 8
                                    AND X.CDTVRA = A.CESLCTD
                                    AND ROWNUM = 1)
                               ESLCTD                      --34 ESTADO_RECLAMO
                            ...
                            /*(SELECT SUBSTR (X.NOFCNA, 1, 100)
                               FROM WEBRECLAMOS.SASMT10 X
                              WHERE X.COFCNA = A.CORGSTRO)*/  ---- no existe A.CORGSTRO
							  null as 
                               ORGSTRO                                    --37
                                      ,
                            --SUBSTR (A.RGSTN, 1, 200) NEJCTVO              --38  //---- no existe A.RGSTN
							null as  NEJCTVO              --38
                       FROM NC_PE_BI.ODS_WRE_SASSL00 A,
                            NC_PE_BI.ODS_WRE_SASSL03 B,
                            NC_PE_BI.ODS_WRE_SASSL03 C
                      WHERE (TO_CHAR (A.FRCPCN, 'MMYYYY') = as_MES || as_ANIO
                             OR TO_CHAR (A.FRSPSTA, 'MMYYYY') =
                                   as_MES || as_ANIO
                             OR A.CESLCTD IN (1, 2) -- 1: ESTADO EN TRAMITE  / 2: ESTADO EN TRAMITE DOCUMENTARIO
